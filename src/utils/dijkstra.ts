...
import { MSTOptimizer } from './mstOptimizer';
...
export class GraphAlgorithms {
  ...
  static findMinimumSpanningTree(graph: Graph): { edges: Array<{from: string, to: string}>, totalWeight: number, optimizedConnections?: Array<{from: string, to: string, segments: Array<{start: {x: number, y: number}, end: {x: number, y: number}}>}> } {
    ...
    if (masterPath && masterPath.points.length > 1) {
      // Usar el nuevo optimizador para conexiones inteligentes
      const optimizedConnections = MSTOptimizer.optimizeConnections(graph.nodes, masterPath);
      
      // Calcular peso total
      let totalWeight = 0;
      for (const connection of optimizedConnections) {
        for (const segment of connection.segments) {
          totalWeight += this.calculateDistance(
      ...
      // Convertir a formato de edges para compatibilidad
      const edges = optimizedConnections
        .filter(conn => conn.to !== 'master' && conn.to !== 'bifurcation')
    ...
    // Sin camino maestro, usar MST tradicional
